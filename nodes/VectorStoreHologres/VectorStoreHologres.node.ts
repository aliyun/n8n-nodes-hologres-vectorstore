...
export function getMetadataFiltersValues(
  ...
  const options = ctx.getNodeParameter("options", itemIndex, {}) as Record<
    string,
    unknown
  >;
...
export async function processDocumentInput(
  ...
  if (Array.isArray(documentInput)) {
    ...
    documentInput &&
    typeof (documentInput as { processItem?: Function }).processItem ===
      "function"
  ) {
    processedDocuments = await (
      documentInput as { processItem: Function }
    ).processItem(inputItem, itemIndex);
  } else {
          ...
          options: [
            {
              ...
              description:
                "Both low and high precision indexes stored in memory",
            },
            {
              name: "Reader IO",
              value: "reader_io",
  ...
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      },
      {
        ...
        placeholder:
          "e.g. Search the Hologres vector store for relevant documents",
        displayOptions: { show: { mode: ["retrieve-as-tool"] } },
      },
      // ── Shared fields ──
      ...sharedFields,
      ...
      },
      {
        ...
        default: 10,
        description:
          "Number of documents to embed in a single batch. Reduce this if your embedding model has batch size limits.",
        displayOptions: { show: { mode: ["insert"] } },
      },
      {
        displayName: "Options",
